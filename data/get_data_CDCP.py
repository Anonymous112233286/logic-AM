...
def get_data(dataset_type):
    mrp_file_path = './data_AM/CDCP/cdcp_' +dataset_type+ '.mrp'  # 请替换为你的文件路径
    output_json_file_path = './data_AM/CDCP/cleaned_data/data.' + dataset_type+ '.json'

    # 读取MRP文件并将每个字典添加到列表中
    with open(mrp_file_path, 'r', encoding='utf-8') as file:
        mrp_list = [json.loads(line.strip()) for line in file]

    plm_path = "./pretrained_model/longformer-base-4096"
    plm_tokenizer =  LongformerTokenizer.from_pretrained(plm_path, add_prefix_space=True)

    ac_label_dict = {'CDCP_value': 'value',
                    'CDCP_fact': 'fact',
                    'CDCP_policy': 'policy',
                    'CDCP_testimony': 'testimony',
                    'CDCP_reference': 'reference',
    }

    ar_label_dict = {'CDCP_reason': 'reason',
                    'CDCP_evidence': 'evidence',
    }
    ...
    dataset = []
    for iii, sample in enumerate(mrp_list):
        if iii == 106:
            print('debug')
        a_data = {}
        try:
        ...
        for a_AC in ACs:
            ...
            span_e = span[0]['to']
            if span_s != str_id:
                text__ = re.sub(r'\s+', ' ', text[str_id:span_s])
                if is_punctuation(text[str_id]):
                    tokens = plm_tokenizer.tokenize(text__.replace('’', "'").replace('‘', "'").replace('“', '"').replace('”', '"').strip(), add_prefix_space=False)
                else:
                    tokens = plm_tokenizer.tokenize(text__.replace('’', "'").replace('‘', "'").replace('“', '"').replace('”', '"').strip(), add_prefix_space=True)
                token_id += len(tokens)
                para_text = para_text.strip() + ' ' + ' '.join(tokens)
                str_id += len(text[str_id:span_s])
            text__ = re.sub(r'\s+', ' ', text[span_s:span_e])
            tokens = plm_tokenizer.tokenize(text__.replace('’', "'").replace('‘', "'").replace('“', '"').replace('”', '"').strip(), add_prefix_space=True)
    ...
    for a_data in dataset:
        ...
        dropped_id = {}
        if len(para_text.split(' ')) > max_len_for_plm:
            exceeded_idx = len(adu_spans)
            for i, span in enumerate(adu_spans):
                if span[0] > max_len_for_plm or span[1] > max_len_for_plm:
